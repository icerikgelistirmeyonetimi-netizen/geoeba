...
(function () {
  ...
  function olc(sahne, kamera) {
    ...
    var esDuzlem = [];
    (function () {
      var Y = [], onbellek = {}, meshSayisi = 0, sz = new T.Vector3(), kutu = new T.Box3();
      ...
      for (var i = 0; i < Y.length; i++) for (var j = i + 1; j < Y.length; j++) {
        ...
        esDuzlem.push({ tur: "EŞ DÜZLEM", a: A.ad, b: B.ad, yuz: yuz, eksen: A.eksen, konum: yuvarla(A.konum),
          y: A.eksen === "y" ? yuvarla(A.konum) : null, ortusme: yuvarla(ort) });   // y: eski alan, yalnız y ekseninde dolu
      }
      esDuzlem.sort(function (p, q) { return q.ortusme - p.ortusme; });
  ...
  window.KONUM3B = { dunyaKutusu: dunyaKutusu, dunyaKaydir: dunyaKaydir, yereOturt: yereOturt, ustuneKoy: ustuneKoy,
